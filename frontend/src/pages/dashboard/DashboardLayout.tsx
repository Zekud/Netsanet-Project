...
export default function DashboardLayout() {
  ...
  const navItems = [
    { to: '/dashboard/home', label: 'Overview', icon: '📊' },
    { to: '/dashboard/cases', label: 'Cases', icon: '📋' },
  ...
  if (user?.role === 'institution_admin' || user?.role === 'system_admin') {
    navItems.push({ to: '/dashboard/staff', label: 'Staff', icon: '👥' });
    navItems.push({ to: '/dashboard/analytics', label: 'Analytics', icon: '📈' });
  }

  if (user?.role === 'system_admin') {
    navItems.push({ to: '/dashboard/institutions', label: 'Institutions', icon: '🏛️' });
  }
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
      ...
      <div className="flex flex-1 flex-col lg:ml-56">
        ...
        <header className="hidden h-14 items-center justify-end border-b border-gray-200 bg-white px-5 lg:flex">
          {user && <NotificationBell userId={user.id} />}
        </header>
